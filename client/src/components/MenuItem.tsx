...
const MenuItem: React.FC<MenuItemProps> = ({ item, quantity, onIncrease, onDecrease }) => {
  ...
  return (
    <div className={`menu-item bg-white rounded-2xl shadow-sm overflow-hidden mb-4 ${isPopular ? 'promo-item' : ''}`}>
          ...
          <div className="flex items-center justify-end mt-auto">
            {quantity > 0 ? (
              <div className="flex items-center bg-accent/50 p-1 rounded-full">
                <button 
                  className="qty-btn w-8 h-8 flex items-center justify-center rounded-full bg-white"
                  onClick={onDecrease}
              ...
              <button 
                className="bg-primary text-white px-4 py-2 rounded-full text-sm font-medium shadow-sm hover:shadow transition-all flex items-center"
                onClick={onIncrease}
